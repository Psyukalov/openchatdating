...
- (void)setSelectedIndex:(NSUInteger)selectedIndex animated:(BOOL)animated {
    if (_labels.count > 0 && selectedIndex <= _labels.count - 1) {
        __weak VPHorizontalPickerView *weakSelf = self;
        dispatch_async(dispatch_get_main_queue(), ^{
            [weakSelf.scrollView setContentOffset:weakSelf.labels[selectedIndex].frame.origin animated:animated];
        });
    }
}

- (void)setFont:(UIFont *)font forState:(VPState)state {
    switch (state) {
        case VPDefaultState:
            _defaultFont = font;
            break;
...
- (void)setColor:(UIColor *)color forState:(VPState)state {
    switch (state) {
        case VPDefaultState:
            _defaultColor = color;
            break;
...
- (void)setSecondFont:(UIFont *)font forState:(VPState)state {
    switch (state) {
        case VPDefaultState:
...
- (void)setSecondColor:(UIColor *)color forState:(VPState)state {
    switch (state) {
        case VPDefaultState:
...
- (void)reloadData {
    if ([_delegate respondsToSelector:@selector(numberOfItemsInPickerView:)]) {
        _count = [_delegate numberOfItemsInPickerView:self];
        if (_count > 0) {
            [self removeData];
            _selectedIndex = 0;
            if ([_delegate respondsToSelector:@selector(marginBetweenItemsInPickerView:)]) {
            ...
            for (NSUInteger i = 0; i <= _count - 1; i++) {
                NSString *string;
                if ([_delegate respondsToSelector:@selector(pickerView:titleForItemAtIndex:)]) {
                    string = [_delegate pickerView:self titleForItemAtIndex:i];
                }
                ...
                if (_useSecondTitle) {
                    NSString *secondString = [NSString new];
                    if ([_delegate respondsToSelector:@selector(pickerView:secondTitleForItemAtIndex:)]) {
...
- (void)removeData {
    ...
    [_secondLabels removeAllObjects];
}
...
- (void)changeSelection:(NSUInteger)selectedIndex animated:(BOOL)animated {
    ...
    if (_useSoundAndFeedback) {
        AudioServicesPlaySystemSound(1306);
        [_feedback selectionChanged];
    }
    [self animate:animated];
    if ([_delegate respondsToSelector:@selector(pickerView:didSelectItemAtIndex:)]) {
        [_delegate pickerView:self didSelectItemAtIndex:_selectedIndex];
    }
}
...
- (void)animate:(BOOL)animated {
    for (NSUInteger i = 0; i <= _labels.count - 1; i++) {
        if (i < _selectedIndex) {
            ...
            [self setFadeWithLabel:_labels[i] withPercent:percent animated:animated];
            if (_useSecondTitle) {
                [self setFadeWithLabel:_secondLabels[i] withPercent:percent animated:animated];
            }
            ...
            [self setFadeWithLabel:_labels[i] withPercent:1.f animated:animated];
            if (_useSecondTitle) {
                [self setFadeWithLabel:_secondLabels[i] withPercent:1.f animated:animated];
            }
        }
    }
}

- (void)setFadeWithLabel:(UILabel *)label withPercent:(CGFloat)percent animated:(BOOL)animated {
    if (animated) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    [self changeSelection:[self findNearestItemIndex] animated:YES];
}

- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView
                  willDecelerate:(BOOL)decelerate {
    if (!decelerate) {
        [_scrollView setContentOffset:_labels[[self findNearestItemIndex]].frame.origin animated:YES];
    }
}
